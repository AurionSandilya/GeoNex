...
async def insert_prediction(
    ...
    latitude: float = 27.55,
    longitude: float = 93.65,
) -> uuid.UUID:
    prediction_id = uuid.uuid4()
    await session.execute(
        text(
            "INSERT INTO risk_predictions (prediction_id, area_id, risk_score, risk_band, confidence, prediction_timestamp, model_version, latitude, longitude, created_at) "
            "VALUES (:pid, :area_id, :risk_score, :risk_band, :confidence, :pts, 'test-model-v1', :lat, :lon, now())"
        ),
        {
            "pid": prediction_id,
            "area_id": area_id,

...
async def _process_one_intent(session: AsyncSession, intent_id) -> None:
    ...
    to_attempt = []  # list of (delivery_id, channel, recipient) - recipient is the full dict from M3
    for recipient in recipients:
        recipient_id = recipient.get("recipient_id")
        # A recipient may want to be reached on more than one channel
        # (e.g. SMS + PUSH) - try every channel in preferred_channels,
        # not just the first one, so PUSH isn't silently skipped.
        channel_strs = recipient.get("preferred_channels") or ([recipient["channel"]] if recipient.get("channel") else [])
        for channel_str in channel_strs:
            ...
            to_attempt.append((delivery.delivery_id, channel, recipient))
    await session.commit()
    ...
    for delivery_id, channel, recipient in to_attempt:
        ...
        if provider is None:
            ...
            continue
        # The provider needs the actual destination address for its channel
        # (phone number for SMS, device token for PUSH) - recipient_id is an
        # internal identifier and is never a valid Twilio/FCM destination.
        ...
        if not destination:
            results.append((
                delivery_id, channel, False,
                ProviderError(f"no {channel.value} destination on file for recipient", FailureType.PERMANENT),
            ))
            continue
        try:
            payload = await provider.send(destination, message)

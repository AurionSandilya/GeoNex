...
class FakeProvider:
    """Stands in for Twilio/FCM. ``send`` receives the resolved destination
    address for the channel (a phone number for SMS, a device token for
    PUSH) - never the internal ``recipient_id`` - matching what the real
    provider wrappers require."""

    def __init__(self, fail: Exception | None = None):
        self.fail = fail
        self.calls: List[tuple] = []

    async def send(self, destination: str, message: str) -> Dict[str, Any]:
        self.calls.append((destination, message))
        if self.fail:
            raise self.fail
        return {"provider_message_id": f"fake-{destination}"}
...
async def test_dispatch_creates_delivery_and_marks_sent(monkeypatch):
    ...
    async def fake_resolve(area_id, alert_id, severity):
        return [{"recipient_id": "rec-1", "phone_number": "+911234567890", "channel": "SMS"}]
...
@pytest.mark.asyncio
async def test_dispatch_never_sends_recipient_id_as_destination(monkeypatch):
    """Regression for the audit's §3.4 finding: the dispatcher must pass
    the resolved phone_number/device_token to the provider, never the
    opaque internal recipient_id, even when recipient_id happens to look
    nothing like a phone number."""
    async with async_session_factory() as session:
        alert, intent = await _make_alert_with_intent(session)

    fake_sms = FakeProvider()
    fake_push = FakeProvider()

    async def fake_resolve(area_id, alert_id, severity):
        return [{
            "recipient_id": "deoc-papum-pare-01",  # internal ID - must never be sent to a provider
    ...
    monkeypatch.setattr(dispatcher_module, "resolve_recipients", fake_resolve)
    monkeypatch.setitem(dispatcher_module._PROVIDERS, DeliveryChannel.SMS, fake_sms)
    monkeypatch.setitem(dispatcher_module._PROVIDERS, DeliveryChannel.PUSH, fake_push)

    processed = await dispatcher_module.dispatch_pending_batch()
    assert processed == 1

    # Both channels in preferred_channels were attempted, not just [0].
    assert len(fake_sms.calls) == 1
    assert len(fake_push.calls) == 1
    sms_destination, _ = fake_sms.calls[0]
    push_destination, _ = fake_push.calls[0]
    assert sms_destination == "+919876543210"
    assert push_destination == "fcm_deoc_papum_pare"
    assert "deoc-papum-pare-01" not in (sms_destination, push_destination)
